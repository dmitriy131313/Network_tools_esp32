...
WebSocketsServer::WebSocketsServer(uint16_t port, const String & origin, const String & protocol)
    ...
    _server->onClient([](void * s, AsyncSSLClient * c) {
        ((WebSocketsServerCore *)s)->newClient(new WEBSOCKETS_NETWORK_CLASS(c));
    },
        this);
#endif
}
...
void WebSocketsServerCore::begin(void) {
    ...
    _runnning = true;
     ...
     DEBUG_WEBSOCKETS("[WS-Server] Websocket Version: %s", WEBSOCKETS_VERSION);
}
...
#if(WEBSOCKETS_NETWORK_TYPE == NETWORK_ESP8266_ASYNC)
            client->tcp->onDisconnect(std::bind([](WebSocketsServerCore * server, WEBSOCKETS_NETWORK_CLASS * obj, WSclient_t * client) -> bool {
                 DEBUG_WEBSOCKETS("[WS-Server] [%d] Disconnect client\n", client->num);

                WEBSOCKETS_NETWORK_CLASS ** sl = &server->_clients[client->num].tcp;
                if(*sl == obj) {
