...
// #define DEBUG_WEBSOCKETS_I(tag, format, ...) ESP_LOGI( tag, format, ##__VA_ARGS__ )
// #define DEBUG_WEBSOCKETS_W(tag, format, ...) ESP_LOGW( tag, format, ##__VA_ARGS__ )
// #define DEBUG_WEBSOCKETS_E(tag, format, ...) ESP_LOGE( tag, format, ##__VA_ARGS__ )

#define DEBUG_WEBSOCKETS(format, ...) ets_printf(format, ##__VA_ARGS__)

// #define DEBUG_WEBSOCKETS_I(tag, format, ...) { void(tag); ets_printf(FGREEN format NONE, ##__VA_ARGS__ ); }
// #define DEBUG_WEBSOCKETS_W(tag, format, ...) { void(tag); ets_printf(FYELLOW format NONE, ##__VA_ARGS__ ); }
// #define DEBUG_WEBSOCKETS_E(tag, format, ...) { void(tag); ets_printf(FRED format NONE, ##__VA_ARGS__ ); }
#else
...
// #endif
// #endif
#define WEBSOCKETS_RECONNECT_TASK_STACK (4 * 1024)
#define WEBSOCKETS_HARTBEAT_TASK_STACK (4 * 1024)

#define WEBSOCKETS_MAX_DATA_SIZE (15 * 1024)
// #define WEBSOCKETS_USE_BIG_MEM
#define GET_FREE_HEAP xPortGetFreeHeapSize()
// moves all Header strings to Flash (~300 Byte)
//#define WEBSOCKETS_SAVE_RAM

#define WEBSOCKETS_YIELD() taskYIELD()
#define WEBSOCKETS_YIELD_MORE() vTaskDelay(1)
...
#define WEBSOCKETS_NETWORK_CLASS AsyncTCP_RW// AsyncTCPbuffer
#define WEBSOCKETS_NETWORK_SERVER_CLASS AsyncSSLServer
...
typedef struct {
    ...
    AsyncSSLClient * tcpclient = nullptr;
