...
WebSocketsClient::WebSocketsClient() {
    ...
    _reconnect_task_h    = nullptr;
    _request_tim_action  = 0;
}

WebSocketsClient::~WebSocketsClient() {
    disconnect();
    //static_cast<FreeRTOS_usr::Timer*>(_reconnect_timer)->stop();
    xTimerStop(_reconnect_timer, portMAX_DELAY);
    xTimerDelete(_reconnect_timer, portMAX_DELAY);
    //delete static_cast<FreeRTOS_usr::Timer*>(_reconnect_timer);
}
...
void WebSocketsClient::begin(const char * host, uint16_t port, const char * url, const char * protocol) {
    ...
    _reconnect_q = xQueueCreate(8, sizeof(uint8_t));
    _reconnect_timer = xTimerCreate("reconn_tim", pdMS_TO_TICKS(_reconnectInterval == 0 ? 1000 : _reconnectInterval), (UBaseType_t )false, this, [](void* arg)
    {
        ets_printf("TIM RECONNECT!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!1\n");
        WebSocketsClient* ptr = (WebSocketsClient*)pvTimerGetTimerID(arg);
        uint8_t snd = WS_HEARTBEAT_EV;
        xQueueSendToBack(ptr->_reconnect_q, &snd, portMAX_DELAY);
    });



    // _reconnect_timer = new FreeRTOS_usr::Timer("reconn_tim", _reconnectInterval == 0 ? 1000 : _reconnectInterval, false, [](void* arg)
    // {
    //     ets_printf("TIM RECONNECT!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!1\n");
    //     WebSocketsClient* ptr = (WebSocketsClient*)pvTimerGetTimerID(arg);
    //     uint8_t snd = WS_HEARTBEAT_EV;
    ...
    xTaskCreatePinnedToCore([](void *o) 
    {
        static UBaseType_t uxHighWaterMark;
        uxHighWaterMark = uxTaskGetStackHighWaterMark(nullptr);
        uint8_t rec;
        WebSocketsClient* ws_ptr =  static_cast<WebSocketsClient*>(o);
        for(;;)
        {
            xQueueReceive(ws_ptr->_reconnect_q, &rec, portMAX_DELAY);
            ws_ptr->asyncConnect(); 
            
            uxHighWaterMark = uxTaskGetStackHighWaterMark(nullptr);
            ets_printf("!!!!!!! Reconnect Tsk Stack!!!!!!!!!!: %u\n", uxHighWaterMark);
        }
    }
    ,"reconn_task"
    , WEBSOCKETS_RECONNECT_TASK_STACK
    , this
    , 5
    , &_reconnect_task_h
    , 1);
    ...
    _lastHeaderSent     = 0;
     ...
     DEBUG_WEBSOCKETS("[WS-Client] Websocket Version: %s", WEBSOCKETS_VERSION);
}
...
void WebSocketsClient::setReconnectInterval(unsigned long time) {
    ...
    {
        xTimerChangePeriod(_reconnect_timer, pdMS_TO_TICKS(_reconnectInterval), portMAX_DELAY);
        //static_cast<FreeRTOS_usr::Timer*>(_reconnect_timer)->changePeriod(_reconnectInterval);
    }
}
...
void WebSocketsClient::handleHeader(WSclient_t * client, String * headerLine) {
    ...
    if(headerLine->length() > 0) {
        ...
        if(ok) {
            ...
            // if(_client.pingInterval != 0) startHeartbeat();
...
#if(WEBSOCKETS_NETWORK_TYPE == NETWORK_ESP8266_ASYNC)
    _client.tcp->onDisconnect(std::bind([](WebSocketsClient * c, WEBSOCKETS_NETWORK_CLASS * obj, WSclient_t * client) -> bool {
         DEBUG_WEBSOCKETS("[WS-Server] [%d] Disconnect client\n", client->num);
        client->status = WSC_NOT_CONNECTED;
        //client->tcp    = NULL; ////////////////////////////////////////////////////////////////// NOT DELETE AsyncTCPbuffer
        client->tcp->clearTCPCallbacks();
        ...
        return false; //////////////////////////////////// false - NOT DELETE AsyncTCPbuffer
    }, this, std::placeholders::_1, &_client));
#endif
    _client.tcp->getAsyncTCP()->onPoll([](void* obj, AsyncSSLClient* client)
    {
        WebSocketsClient* ws = ((WebSocketsClient*)(obj));
        ws->handleHBPing();
        ws->handleHBTimeout(&ws->_client);

        static int cnt = 0;
        if (ws->_client.request_disconnect/* && cnt++ > 5*/)
        {
            ets_printf("___________zzzzzzzzz_________DISCONNECT!!!!!!!!!!!!!!!!!!!!!!\n");
            ws->clientDisconnect(&ws->_client);
            ws->_client.request_disconnect = false;
            cnt = 0;
        }
...
void WebSocketsClient::connectFailedCb() {
    // stopHeartbeat();
    reconnectTimStart();
    //DEBUG_WEBSOCKETS("[WS-Client] connectFailedCb %s:%u Failed\n", _host.c_str(), _port);
    DEBUG_WEBSOCKETS("[WS-Client] connectFailedCb\n");
}

#if(WEBSOCKETS_NETWORK_TYPE == NETWORK_ESP8266_ASYNC)

void WebSocketsClient::asyncConnect() {
    ets_printf("____________________tcpclient CONNECT   minimum: %d;   current: %d\n", xPortGetMinimumEverFreeHeapSize(), xPortGetFreeHeapSize());
    DEBUG_WEBSOCKETS("[WS-Client] asyncConnect...\n");
    if (_client.tcpclient == nullptr) _client.tcpclient = new AsyncSSLClient();
    //AsyncSSLClient * tcpclient = new AsyncSSLClient();

    if(!_client.tcpclient) {
        DEBUG_WEBSOCKETS("[WS-Client] creating AsyncSSLClient class failed!\n");
        connectFailedCb();
        return;
    }

    _client.tcpclient->onDisconnect([](void * obj, AsyncSSLClient * c) {
        WebSocketsClient* ws = ((WebSocketsClient*)(obj));
        // delete c;
        ets_printf("____________________tcpclient DISCONNECT   minimum: %d;   current: %d\n", xPortGetMinimumEverFreeHeapSize(), xPortGetFreeHeapSize());
        ws->connectFailedCb();
    }, this);

    _client.tcpclient->onConnect(std::bind([](WebSocketsClient * ws, AsyncSSLClient * tcp) {
        //if (ws->_client.tcp != nullptr) ::abort();

        if (ws->_client.tcp == nullptr) 
        {
            ws->_client.tcp = new WEBSOCKETS_NETWORK_CLASS(tcp);
            if(!ws->_client.tcp) {
               DEBUG_WEBSOCKETS("[WS-Client] creating Network class failed!\n");
                // delete tcp;
                ws->connectFailedCb();
        ...
        {
            ws->_client.tcp->updateTCPClient(tcp);
        }
        ws->connectedCb();
        
    }, this, std::placeholders::_2));

    // tcpclient->onError(std::bind([](WebSocketsClient * ws, AsyncSSLClient * tcp) {
    //     // delete tcp;
    //     // ws->connectFailedCb();
    ...
    if(!_client.tcpclient->connect(_host.c_str(), _port, true)) {
        // delete tcpclient;
        connectFailedCb();
    }
}

void WebSocketsClient::reconnectTimStart()
{
    if (_reconnectInterval > 0)
    {
        xTimerStart(_reconnect_timer, portMAX_DELAY);// static_cast<FreeRTOS_usr::Timer*>(_reconnect_timer)->start();
    }
}

void WebSocketsClient::reconnectTimStop()
{
    _request_tim_action = 0;
    xTimerStop(_reconnect_timer, portMAX_DELAY);
    //static_cast<FreeRTOS_usr::Timer*>(_reconnect_timer)->stop();
...
// void WebSocketsClient::startHeartbeat()
// {
//     if (_heartbeat_task_h == nullptr) 
//         xTaskCreatePinnedToCore([](void *o) { static_cast<WebSocketsClient*>(o)->heartbeat_task(nullptr);} ,"heartbeat_task", 4096, this, 5, &_heartbeat_task_h, 1);
// }

// void WebSocketsClient::stopHeartbeat()
// {
//     if (_heartbeat_task_h != nullptr)
//     {
//         vTaskSuspend(_heartbeat_task_h);
//         vTaskDelete(_heartbeat_task_h);
//         _heartbeat_task_h = nullptr;
//     }
// }

///////////////////////////////////////////////////////////////////////////////////

// void WebSocketsClient::heartbeat_task(void* arg)
// {
//     static UBaseType_t uxHighWaterMark;
//     uxHighWaterMark = uxTaskGetStackHighWaterMark(nullptr);
//     for(;;)
...
//         handleHBPing();
//         handleHBTimeout(&_client);
//         ets_printf("!!!!!!! heartbeat Tsk Stack!!!!!!!!!!: %u\n", uxHighWaterMark);
