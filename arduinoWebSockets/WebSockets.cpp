...
String WebSockets::base64_encode(uint8_t * data, size_t length) {
    ...
    // char * buffer = (char *)malloc(size); //OLD
    if(1 /*buffer*/) {
        base64_encodestate _state;
        base64_init_encodestate(&_state);
        int len = base64_encode_block((const char *)&data[0], length, &buffer[0], &_state);
        // len     = base64_encode_blockend((buffer + len), &_state); //OLD
        len     = base64_encode_blockend((&buffer[len]), &_state); //NEW

        // String base64 = String(buffer); //OLD
        String base64 = String(buffer.data()); //NEW
        //free(buffer); //OLD
        return base64;
    }
    return String("-FAIL-");
    ...
    if(client->pingInterval) {    // if heartbeat is enabled
        ...
        if(client->pongReceived) {
            ...
            if(pi > client->pongTimeout) {    // pong not received in time
                ...
                DEBUG_WEBSOCKETS("[HBtimeout] pong TIMEOUT! lp=%u millis=%u pi=%u count=%u\n", (uint32_t)client->lastPing, (uint32_t)millis(), (uint32_t)pi, (uint32_t)client->pongTimeoutCount);
