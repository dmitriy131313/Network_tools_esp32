...
AsyncTCPbuffer::AsyncTCPbuffer(AsyncSSLClient* client) {
    if(client == NULL) {
    ...
    _RXbuffer = new /*(std::nothrow)*/ cbuf(300);
...
void AsyncTCPbuffer::updateClient(AsyncSSLClient* c)
{
    if(c == NULL) {
        DEBUG_ASYNC_TCP("[A-TCP] client is null!!!\n");
        ::abort();
...
/**
 * attachCallbacks to AsyncSSLClient class
 */
void AsyncTCPbuffer::_attachCallbacks() {
    ...
    _client->onData([](void *obj, AsyncSSLClient* c, void *buf, size_t len) {
        (void)c;
        AsyncTCPbuffer* b = ((AsyncTCPbuffer*)(obj));
        b->_rxData((uint8_t *)buf, len);
    }, this);

    _client->onTimeout([](void *obj, AsyncSSLClient* c, uint32_t time){
        (void)obj;
        (void)time;
...
size_t AsyncTCPbuffer::_handleRxBuffer(uint8_t *buf, size_t len) {
    ...
    if(_RXmode == ATB_RX_MODE_NONE) {
        ...
        if(BufferAvailable > 0) {
            ...
            if(b == NULL){
              abort(); //TODO: What action should this be ?
            }
