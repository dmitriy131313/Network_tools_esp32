...
#define TCP_SSL_DEBUG ets_printf
...
int tcp_ssl_send(void *ctx, const unsigned char *buf, size_t len)
{
    ...
    TCP_SSL_DEBUG("tcp_ssl_send: tcp_write(%x, %x, %d, %x)\n", (uint32_t)tcp_ssl->tcp, (uint32_t)buf, tcp_len, (uint32_t)tcp_ssl->arg);
...
int tcp_ssl_new_client(struct tcp_pcb *tcp, void *arg, const char *hostname, const char *root_ca, const size_t root_ca_len,
                       const char *cli_cert, const size_t cli_cert_len, const char *cli_key, const size_t cli_key_len, void* extArg)
{
    tcp_ssl_t *tcp_ssl;
...
int tcp_ssl_new_psk_client(struct tcp_pcb *tcp, void *arg, const char *psk_ident, const char *pskey, void* extArg)
{
    tcp_ssl_t *tcp_ssl;
...
int tcp_ssl_read(struct tcp_pcb *tcp, struct pbuf *p, void* extArg)
{
    TCP_SSL_DEBUG("tcp_ssl_read(%x, %x)\n", (uint32_t)tcp, (uint32_t)p);
    ...
    do
    {
        if (tcp_ssl->ssl_ctx.state != MBEDTLS_SSL_HANDSHAKE_OVER)
        {
            if (tcp_ssl_get(tcp) == NULL)
            {
                TCP_SSL_DEBUG("tcp_ssl_read: tcp is NULL; Return\n");
                return -1;
            }
            // KH to print once
            if (!debugPrinted)
            ...
            if (ret == 0)
            {
                ...
                //////
                TCP_SSL_DEBUG("Verifying peer X.509 certificate... %u\n", (uint32_t)&tcp_ssl->ssl_ctx);

                if ((mbedtls_ssl_get_verify_result(&tcp_ssl->ssl_ctx)) != 0)
                {
                    TCP_SSL_DEBUG("handshake error: %d\n", ret);
                    TCP_SSL_DEBUG("________1________\n");
                    handle_error(ret);
                    err = ret;
